...
public class DestinationListFragmentTest extends ActivityInstrumentationTestCase2<TabbedSummaryClaimantActivity> {
	...
	/**
	 * Setup for each test. Creates a new claim and adds a destination. It then
	 * starts the activity with the intent of displaying info about this claim
	...
	/**
	 * Checks that we can access views within the fragment. If this test fails,
	 * the fragment isn't being put into the activity correctly.
	 */
	public void testFragViewsNotNothing() {
		// Try to fetch the list view in the fragment.
		assertNotNull(frag.getView().findViewById(R.id.destinationsListView));
	}
	...
	/**
	 * Test that deleting an existing destination removes it from the list view.
	 */
